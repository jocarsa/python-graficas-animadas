...
output_path = os.path.join(output_dir, f'{int(time.time())}_stylized_inverted_control_panel_animation.mp4')
...
def update(frame):
    ...
    for i, ax in enumerate(axes.flat):
        ...
        if i == 0:
            ...
            ax.clear()
            ax.plot(scatter_data[0], scatter_data[1], 'o', color=color, alpha=0.7)  # Update scatter plot
            ax.set_title(plot_titles[i], fontsize=12, fontweight='bold', color='white')
            ax.grid(True, color=base_color * 0.2)
            ax.set_facecolor('black')
        elif i == 3:
            ...
            random_noise = np.random.randn(100)
            ax.clear()
            ax.hist(random_noise, bins=20, range=(-3, 3), color=color, alpha=0.7)  # Update histogram
            ax.set_title(plot_titles[i], fontsize=12, fontweight='bold', color='white')
            ax.grid(True, color=base_color * 0.2)
            ax.set_facecolor('black')
        elif i == 5:
            ax.clear()
            ax.pie([10, 20, 30, 40], startangle=frame, autopct='%1.1f%%', colors=[color, color*0.8, color*0.6, color*0.4])  # Update pie chart
            ax.set_title(plot_titles[i], fontsize=12, fontweight='bold', color='white')
        elif i == 6:
            ...
            ax.clear()
            ax.plot(np.sin(x + frame * 0.1), np.cos(x + frame * 0.1), color=color, lw=2)  # Update spiral
            ax.set_title(plot_titles[i], fontsize=12, fontweight='bold', color='white')
            ax.grid(True, color=base_color * 0.2)
            ax.set_facecolor('black')
        elif i == 10:
            ax.clear()
            ax.plot(x, (x + frame * 0.01)**2, color=color, lw=2)  # Update quadratic
            ax.set_title(plot_titles[i], fontsize=12, fontweight='bold', color='white')
            ax.grid(True, color=base_color * 0.2)
            ax.set_facecolor('black')
        elif i == 11:
